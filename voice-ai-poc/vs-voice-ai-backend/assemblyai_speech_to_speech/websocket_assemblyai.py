...
from .constants_assemblyai import SYSTEM_INSTRUCTION_VOICE
from .langchain_assemblyai import AssemblyAIVoiceReactAgent
...
@router.websocket("/media-stream-assemblyai")
async def websocket_endpoint(websocket: WebSocket):
    ...
    # Get agent_id from environment variable (optional - will create if not provided)
    agent_id = os.getenv('ASSEMBLYAI_AGENT_ID', '')
    agent_name = os.getenv('ASSEMBLYAI_AGENT_NAME', 'Petvisor_Voice_Agent')

    agent = AssemblyAIVoiceReactAgent(
