...
echo "Starting backend (uv run server.py)..."
(
  cd "${ROOT_DIR}"
  uv run server.py
) &
...
ALLOW_UI_PORT_FALLBACK="${ALLOW_UI_PORT_FALLBACK:-true}"
UI_PORT="${PREFERRED_UI_PORT}"
if is_port_in_use "${UI_PORT}"; then
  if [[ "${STRICT_UI_PORT}" == "true" || "${ALLOW_UI_PORT_FALLBACK}" != "true" ]]; then
